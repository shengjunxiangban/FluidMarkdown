            ...
            if (type == CMNodeTypeTable) {
                ...
                if (event == CMEventTypeEnter) {
                    if (_delegateFlags.didStartTableCell) {
                        NSTextAlignment alignment = node.cellAlignment;
                        [_delegate parser:self didStartTableCellWithAlignment:alignment];
                    }
                } else {
                    if (_delegateFlags.didEndTableCell) {
                        NSTextAlignment alignment = node.cellAlignment;
                        [_delegate parser:self didEndTableCellWithAlignment:alignment];
                    }
                }
...
- (void)setDelegate:(id<CMParserDelegate>)delegate
{
    if (_delegate != delegate) {
        ...
        _delegateFlags.didStartTableRow = [_delegate respondsToSelector:@selector(parser:didStartTableRowIsHeader:)];
